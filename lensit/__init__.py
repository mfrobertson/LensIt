...
def get_fidcls(ellmax_sky=LMAX_SKY, wrotationCls=False):
    ...
    """
    cls_unl = {}
    cls_unlr = camb_clfile(os.path.join(_get_lensitdir()[1], 'my_fiducial_flatsky_lenspotentialCls.dat')) #fiducial_flatsky_lenspotentialCls.dat
...
def get_lencmbs_lib(res=14, wrotation=False, cache_sims=True, nsims=120, num_threads=int(os.environ.get('OMP_NUM_THREADS', 1))):
    ...
    skypha_libdir = os.path.join(_get_lensitdir()[0], 'temp', '%s_sims' % nsims, 'fsky%04d' % fsky, 'len_alms',
                                 'skypha' + wrotation * '_wcurl')
